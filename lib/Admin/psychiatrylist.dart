...
class PsychiatryList extends StatefulWidget {
  @override
  PsychiatryListState createState() => PsychiatryListState();
}

class PsychiatryListState extends State<PsychiatryList> {
  @override
  var id;
  var name;
  List psychiatrylist = [];
  ...
  fetchUser() async {
    ...
    var url =
        "http://hospitalmanagement.educationhost.cloud/psychiatrylist.php";
    var response = await http.get(Uri.parse(url));
    print(response.body);
    if (response.statusCode == 200) {
      var items = json.decode(response.body);
      setState(() {
        psychiatrylist = items;
        isLoading = false;
      });
    } else {
      psychiatrylist = [];
  ...
  Widget listBody() {
    if (psychiatrylist.contains(null) ||
        psychiatrylist.length < 0 ||
        isLoading) {
      return Center(
    ...
    return ListView.builder(
        itemCount: psychiatrylist.length,
        shrinkWrap: true,
        itemBuilder: (context, index) {
          return psychiatrycard(psychiatrylist[index]);
        });
  }

  Widget psychiatrycard(items) {
    name = items['name'];
